...
/// The Lstm module. This implementation is for unidirectional or bidirectional Lstm.
#[derive(Module, Debug)]
pub struct Lstm<B: Backend> {
    ...
    input_gate_bw: Option<GateController<B>>,
    forget_gate_bw: Option<GateController<B>>,
    output_gate_bw: Option<GateController<B>>,
    cell_gate_bw: Option<GateController<B>>,
    d_hidden: usize,
}
...
impl LstmConfig {
    ...
    pub fn init<B: Backend>(&self) -> Lstm<B> {
        ...
        let new_gate =
            || GateController::new(self.d_input, d_output, self.bias, self.initializer.clone());

        let (input_gate_bw, forget_gate_bw, output_gate_bw, cell_gate_bw);
    ...
    pub fn init_with<B: Backend>(&self, record: LstmRecord<B>) -> Lstm<B> {
        ...
        let (mut input_gate_bw, mut forget_gate_bw, mut output_gate_bw, mut cell_gate_bw) =
            (None, None, None, None);
        if let (Some(ig_bw_record), Some(fg_bw_record), Some(cg_bw_record), Some(og_bw_record)) = (
            record.input_gate_bw,
            record.forget_gate_bw,
            record.cell_gate_bw,
            record.output_gate_bw,
        ) {
            if self.bidirectional {
                (input_gate_bw, forget_gate_bw, output_gate_bw, cell_gate_bw) = (
                    Some(GateController::new_with(&linear_config, ig_bw_record)),
                    Some(GateController::new_with(&linear_config, fg_bw_record)),
                    Some(GateController::new_with(&linear_config, og_bw_record)),
                    Some(GateController::new_with(&linear_config, cg_bw_record)),
                )
            }
        }
...
impl<B: Backend> Lstm<B> {
    ...
    pub fn forward(
        ...
        let num_directions = if let (Some(_), Some(_), Some(_), Some(_)) = (
            &self.input_gate_bw,
            &self.forget_gate_bw,
            &self.output_gate_bw,
            &self.cell_gate_bw,
        ...
        for (input_t, t) in batched_input.clone().iter_dim(1).zip(0..) {
            let (unsqueezed_cell_state, unsqueezed_hidden_state) =
                self.apply_one_direction(input_t, &mut cell_state, &mut hidden_state, false);
        ...
        if num_directions == 2 {
            ...
            for (input_t, t) in batched_input.iter_dim(1).rev().zip((0..seq_length).rev()) {
                let (unsqueezed_cell_state, unsqueezed_hidden_state) =
                    self.apply_one_direction(input_t, &mut cell_state, &mut hidden_state, true);

                batched_cell_state = batched_cell_state.slice_assign(
                    [0..batch_size, t..(t + 1), self.d_hidden..self.d_hidden * 2],
                    unsqueezed_cell_state.clone(),
                );
                batched_hidden_state = batched_hidden_state.slice_assign(
                    [0..batch_size, t..(t + 1), self.d_hidden..self.d_hidden * 2],
    ...
    fn apply_one_direction(
        &self,
        input_t: Tensor<B, 3>,
        cell_state: &mut Tensor<B, 2>,
        hidden_state: &mut Tensor<B, 2>,
        backward: bool,
    ) -> (Tensor<B, 3>, Tensor<B, 3>) {
        let input_t = input_t.squeeze(1);

        let (forget_gate, input_gate, output_gate, cell_gate);
        if backward {
            (forget_gate, input_gate, output_gate, cell_gate) = (
                self.forget_gate_bw.as_ref().unwrap(),
                self.input_gate_bw.as_ref().unwrap(),
                self.output_gate_bw.as_ref().unwrap(),
                self.cell_gate_bw.as_ref().unwrap(),
            )
        } else {
            (forget_gate, input_gate, output_gate, cell_gate) = (
                &self.forget_gate,
        ...
        *hidden_state = output_values * cell_state.clone().tanh();
        ...
        (unsqueezed_cell_state, unsqueezed_hidden_state)
    }
...
mod tests {
    ...
    fn test_bidirectional() {
        ...
        let input = Tensor::<TestBackend, 3>::from_data(Data::from([[
            [-0.131, -1.591],
            [1.378, -1.867],
            [0.397, 0.047],
        ]]));
